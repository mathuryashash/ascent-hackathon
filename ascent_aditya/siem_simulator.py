...
def fire_webhook(log_line: str, matched_pattern: str) -> None:
    ...
    try:
        ...
        print(f"[SIEM] ⚠️  Webhook failed (orchestrator may not be up yet): {e.reason} | Alert ID: {alert_id}")
    except Exception as e:
        print(f"[SIEM] ⚠️  Unexpected error firing webhook: {e} | Alert ID: {alert_id}")
...
def tail_log(path: str) -> None:
    ...
    print(f"[SIEM] 🛡️  Watching for patterns: {[p for p, _ in COMPILED_PATTERNS]}")
    print("-" * 70)
